...
(function() {
  document.addEventListener('turbolinks:load', function() {
    Turbolinks.clearCache();
    $("#staff-location-switcher").on("change", function() {
      var location_id;
    ...
    $("#child-location-switcher").on("change", function() {
      var location_id, program_id;
      location_id = $(this).val();
    ...
    return $("#child-program-switcher").on("change", function() {
      var location_id, program_id;
      program_id = $(this).val();
...
(function() {
  document.addEventListener('turbolinks:load', function() {
    Turbolinks.clearCache();
    return $('form').on('cocoon:after-insert', function() {
...
(function() {
  var updateDropIn, updateWeeklyPlan;

  updateWeeklyPlan = function(elem) {
    var chunks, start_str, stop_str, value;
    value = elem.val();
    chunks = value.split(" to ");
    start_str = chunks[0];
    stop_str = chunks[1];
    elem.parent().find(".weekly-starts").val(start_str);
    return elem.parent().find(".weekly-ends").val(stop_str);
  };

  updateDropIn = function(elem) {
    var date, day_num, value;
    value = elem.val();
    elem.parent().find(".drop_in-ends").val(value);
    date = new Date(value);
    day_num = date.getUTCDay();
    elem.parent().find(".drop_in-days").val(false);
    return elem.parent().find(".drop_in-" + day_num).val(true);
  };

  document.addEventListener('turbolinks:load', function() {
    $(".weekly_select").each(function() {
      return updateWeeklyPlan($(this));
    });
    $(".plan-box").on("change", ".weekly-select", function(e) {
      return updateWeeklyPlan($(this));
    });
    $(".drop_in-date-select").each(function() {
      return updateDropIn($(this));
    });
    $(".account-box").on("change", ".drop_in-date-select", function(d) {
      return updateDropIn($(this));
    });
    $(".account-box").on("click", ".add-day-button", function(e) {
      return setTimeout((function() {
        return $(".account-box").find(".drop_in-date-select").each(function() {
          return updateDropIn($(this));
        });
      }), 10);
    });
    $(".enrollment-pricing-table").on("keyup", ".addable-amount", function(event) {
      var charge_total, elem, refund_total;
      elem = $(event.currentTarget);
      charge_total = 0.0;
      refund_total = 0.0;
      $(".enrollment-pricing-table").find(".addable-amount").each(function() {
        var amount;
        amount = parseFloat($(this).val().replace(/[^0-9\.\-]/, ""));
        if (amount < 0.0) {
          return refund_total += amount * -1;
        } else {
          return charge_total += amount;
        }
      });
      ...
      if (refund_total > 0) {
        ...
        return $("#refund-block").removeClass("hide");
      } else {
        return $("#refund-block").addClass("hide");
      }
    });
    $(".enrollment-pricing-table").on("change", ".change-refund-requirement", function(event) {
      var change_id, elem, require_refund;
      elem = $(event.currentTarget);
    ...
    return $(".enrollment-pricing-table").on("change", ".change-fee-requirement", function(event) {
      var account_id, elem, program_id, require_fee;
      elem = $(event.currentTarget);
...
function addFieldToForm(form, token, field) {
  var hiddenInput = document.createElement('input');
  hiddenInput.setAttribute('type', 'hidden');
  hiddenInput.setAttribute('name', "account[card_" + field + "]");
...
(function() {
  document.addEventListener('turbolinks:load', function() {
    return $(".staff-time-entries").on("change", ".clock-toggle", function(event) {
      var clock_str, elem, location_id, on_clock, pin_number, pin_number_elem, time_recordable_id, time_recordable_type;
      elem = $(event.currentTarget);
      ...
      return $.ajax({
        ...
        success: function(data) {
          console.log("success!");
          pin_number_elem.val("");
          return elem.prop('checked', data["clocked_in"]);
        },
        error: function(xhr, status, error) {
          var error_div;
          console.log(xhr);
          ...
          if (xhr.responseText === "PIN Number Incorrect") {
            ...
            return console.log("xhr response " + xhr.responseText + " incorrect");
          }
        }
      });
    });
