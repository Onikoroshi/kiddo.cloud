class Enrollment < ApplicationRecord
  ...
  scope :inactive, -> { where("enrollments.starts_at > ? OR enrollments.ends_at < ?", Time.zone.today, Time.zone.today).distinct }
  scope :require_payment_information, -> { joins(:program).where(programs: {waive_payment_information: false}) }
  ...
  def to_s
    ...
    when PlanType[:custom_request].to_s
      if program.custom_requests_url.present?
        ...
        "Contact our office at <a href='mailto:office@daviskidsklub.com'>office@daviskidsklub.com</a> to talk with us about your custom enrollment request.".html_safe
      end
    when PlanType[:weekly].to_s
  ...
  def to_short
    ...
    when PlanType[:drop_in].to_s
      ...
      "#{"#{plan.display_name} " unless plan.deduceable?}#{plan.plan_type.text} plan on #{enrolled_days(humanize: true)}#{(ends_at > starts_at) ? " from #{service_dates}" : ", #{starts_at.stamp("Feb. 3rd, 2018")}"} at #{location_name}."
    end
  end
