class Staff::LocationsController < ApplicationController
  ...
  def update_availables
    @location.program_locations.each do |prog_loc|
      ...
      prog_loc.update_attribute(:enable_alerts, params["enable_alerts"].present? && params["enable_alerts"][prog_loc.program_id.to_s].present? && params["enable_alerts"][prog_loc.program_id.to_s] == "true")
    end
  end
